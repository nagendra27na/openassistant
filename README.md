...
Quickstart · Features · Install · FAQ · Roadmap · Contributing
</div>
---
What is OpenAssistant?
OpenAssistant is an open-source AI assistant that runs entirely on your own machine using Ollama and open-weight models like Llama 3.1. It has a web UI and a terminal CLI, supports multiple assistant modes, and costs nothing to run after setup.
...
Go to https://ollama.com and install it for your OS.
...
```
For full OS-specific instructions see Installation below.
---
Installation
macOS
```bash
...
python app.py
```
Open http://localhost:5000. Use the sidebar to switch modes or start a new conversation.
CLI
```bash
...
Yes — any model in the Ollama library. Change `MODEL` in `config.py` and pull the model first.
...
Have a feature idea? Open an issue and describe it.
---
Development
...
Start a thread in GitHub Discussions to show a fork or customization
...
Open an issue describing what you want and why it's useful. Check the Roadmap first — it may already be planned.
Submitting a pull request
Fork the repo and clone your fork
Create a branch: `git checkout -b fix/your-fix` or `feature/your-feature`
Make your changes
Verify everything still works: `python check.py`, `python cli.py`, `python app.py`
...
---
License
MIT — use it, fork it, build on it, sell it. No restrictions.
The AI models downloaded via Ollama have their own licenses. Llama 3.1 uses Meta's community license, Mistral and Qwen 2.5 use Apache 2.0. See the Ollama model library for details on each model's license.
---
<div align="center">
